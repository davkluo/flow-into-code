...
import { GET, POST } from "@/app/api/problems/[slug]/preview/route";
...
vi.mock("@/lib/rateLimit", () => ({
  ...
  getGeneratePreviewRateLimit: vi.fn(),
}));
vi.mock("@/services/previewData", () => ({
  getPreviewData: vi.fn(),
  generatePreviewData: vi.fn(),
}));

const mockLimit = vi.fn();

const makeRequest = (method = "GET") =>
  new NextRequest("http://localhost/api/problems/two-sum/preview", { method });
...
beforeEach(() => {
  ...
  vi.mocked(rateLimit.getGeneratePreviewRateLimit).mockReturnValue({
    limit: mockLimit,
  } as unknown as Ratelimit);
  mockLimit.mockResolvedValue({ success: true, reset: Date.now() + 60000 });
});

describe("GET /api/problems/[slug]/preview", () => {
  ...
  it("returns 200 with long-lived cache header when complete", async () => {
    ...
    expect(body.titleSlug).toBe("two-sum");
  });

  it("returns 202 when processing", async () => {
    vi.mocked(verifyFirebaseToken).mockResolvedValue("uid");
    vi.mocked(getPreviewData).mockResolvedValue({ status: "processing" });
    const res = await GET(makeRequest(), makeParams("two-sum"));
  ...
  it("returns 404 when not found", async () => {
    vi.mocked(verifyFirebaseToken).mockResolvedValue("uid");
    vi.mocked(getPreviewData).mockResolvedValue({ status: "not_found" });
    const res = await GET(makeRequest(), makeParams("two-sum"));
  ...
  it("returns 500 on unexpected error", async () => {
    vi.mocked(verifyFirebaseToken).mockResolvedValue("uid");
    vi.mocked(getPreviewData).mockRejectedValue(new Error("db error"));
    const res = await GET(makeRequest(), makeParams("two-sum"));
    expect(res.status).toBe(500);
  });
});

describe("POST /api/problems/[slug]/preview", () => {
  it("returns 401 when token is invalid", async () => {
    vi.mocked(verifyFirebaseToken).mockResolvedValue(null);
  ...
  it("returns 202 when generation is still in progress", async () => {
    vi.mocked(verifyFirebaseToken).mockResolvedValue("uid");
    vi.mocked(generatePreviewData).mockResolvedValue(null);
    const res = await POST(makeRequest("POST"), makeParams("two-sum"));
    expect(res.status).toBe(202);
    const body = await res.json();
    expect(body).toEqual({ status: "processing" });
  ...
  it("returns 200 with generated data", async () => {
    vi.mocked(verifyFirebaseToken).mockResolvedValue("uid");
    vi.mocked(generatePreviewData).mockResolvedValue({
      titleSlug: "two-sum",
    } as ProblemDetails);
    const res = await POST(makeRequest("POST"), makeParams("two-sum"));
    expect(res.status).toBe(200);
  ...
  it("returns 500 on unexpected error", async () => {
    vi.mocked(verifyFirebaseToken).mockResolvedValue("uid");
    vi.mocked(generatePreviewData).mockRejectedValue(new Error("fail"));
    const res = await POST(makeRequest("POST"), makeParams("two-sum"));
    expect(res.status).toBe(500);
  });
});
