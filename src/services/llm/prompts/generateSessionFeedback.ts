...
export const GENERATE_SESSION_FEEDBACK_PROMPT_VERSION = 1;
...
- Weak: jumps directly to an optimal solution without explaining the reasoning chain; cannot articulate the complexity of their stated approach; changes strategies without noting why the previous one was abandoned.
`.trim(),
...
- advice: specific to this candidate's gaps. Name what was actually missing or wrong, not a generic tip.
...
1. interviewerCommunication — Assess how the candidate used the AI interviewer across the session.
   Consider: did they ask well-framed clarifying questions or validation-seeking ones? Did they debug purposefully or lean on the interviewer to find bugs? Did they work independently or request hints repeatedly?
   ...
   If they used it minimally, score on whatever did occur — working silently is not a negative, but it also cannot be evaluated positively.

2. summary — 2–4 sentences on the candidate's overall performance. Name the strongest section, the biggest gap, and any consistent pattern. Be direct — do not restate the individual scores.
